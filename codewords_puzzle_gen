...
def generate_codewords_puzzle(word_list, grid_size):
    ...
    coded_grid = [[letter_to_number[grid[r][c]] if grid[r][c] != ' ' else '.' for c in range(grid_size)] for r in range(grid_size)]
    hints = {letter: letter_to_number[letter] for letter in random.sample(letters, max(1, len(letters) // 4))}
    
    return coded_grid, hints, letter_to_number, grid
